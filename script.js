...
// **** 薬剤データ定義 (最終データ、変更なし) ****
const DRUG_DATA = [
    { category: '心血管作動薬', name: 'ドブタミン100mg/5mL', code: 'DobutamineH', conc_mg_mL: 20.0, target_dose: 5.0, target_unit: 'ug/kg/min' },
    { category: '心血管作動薬', name: 'ドブタミン0.3%シリンジ', code: 'DobutamineL', conc_mg_mL: 3.0, target_dose: 5.0, target_unit: 'ug/kg/min' },
    { category: '心血管作動薬', name: 'ドパミン100mg/5mL', code: 'DopamineH', conc_mg_mL: 20.0, target_dose: 5.0, target_unit: 'ug/kg/min' },
    { category: '心血管作動薬', name: 'イノバン0.3%シリンジ', code: 'DopamineL', conc_mg_mL: 3.0, target_dose: 5.0, target_unit: 'ug/kg/min' },
    { category: '心血管作動薬', name: 'ノルアドレナリン1mg/mL', code: 'Noradrenaline', conc_mg_mL: 1.0, target_dose: 0.05, target_unit: 'ug/kg/min' },
    { category: '心血管作動薬', name: 'アドレナリン1mg/mL', code: 'Adrenaline', conc_mg_mL: 1.0, target_dose: 0.05, target_unit: 'ug/kg/min' },
    { category: '心血管作動薬', name: 'ミルリノン10mg/10mL', code: 'Millinon', conc_mg_mL: 1.0, target_dose: 0.5, target_unit: 'ug/kg/min' },
    { category: '心血管作動薬', name: 'アルプロスタジルアルファデックス(PGE1-CD)', code: 'PGE1CD', conc_mg_mL: 0.020, target_dose: 50.0, target_unit: 'ng/kg/min', stock_note: '1Vを生食 1mL で溶解' },
    { category: '心血管作動薬', name: 'パルクス(Lipo-PGE1)', code: 'LipoPGE1', conc_mg_mL: 0.005, target_dose: 5.0, target_unit: 'ng/kg/min' },
    { category: '心血管作動薬', name: 'ピトレシン20U', code: 'Vasopresin', conc_mg_mL: 20.0, target_dose: 0.4, target_unit: 'mU/kg/min' },
    { category: '鎮静薬', name: '10kg以上：ミダゾラム', code: 'Midazolamb10', conc_mg_mL: 5.0, target_dose: 0.1, target_unit: 'mg/kg/hr' },
    { category: '鎮静薬', name: '10kg未満：ミダゾラム', code: 'Midazolamu10', conc_mg_mL: 5.0, target_dose: 0.2, target_unit: 'mg/kg/hr' },
    { category: '鎮痛薬', name: '10kg以上：フェンタニル', code: 'Fentanylb10', conc_mg_mL: 0.05, target_dose: 1.0, target_unit: 'ug/kg/hr' },
    { category: '鎮痛薬', name: '10kg未満：フェンタニル', code: 'Fentanylu10', conc_mg_mL: 0.05, target_dose: 2.0, target_unit: 'ug/kg/hr' },
    { category: '鎮痛薬', name: '10kg以上：モルヒネ', code: 'Morphineb10', conc_mg_mL: 10.0, target_dose: 20.0, target_unit: 'ug/kg/hr' },
    { category: '鎮痛薬', name: '10kg未満：モルヒネ', code: 'Morphineu10', conc_mg_mL: 10.0, target_dose: 40.0, target_unit: 'ug/kg/hr' },
    { category: '筋弛緩薬', name: '10kg未満：ロクロニウム', code: 'Rocuroniumu10', conc_mg_mL: 10.0, target_dose: 10.0, target_unit: 'ug/kg/min' },
    { category: '筋弛緩薬', name: '10-30kg：ロクロニウム', code: 'Rocuronium1030', conc_mg_mL: 10.0, target_dose: 5.0, target_unit: 'ug/kg/min' },
    { category: '筋弛緩薬', name: '30kg以上：ロクロニウム', code: 'Rocuroniumb30', conc_mg_mL: 10.0, target_dose: 3.0, target_unit: 'ug/kg/min' },
    { category: '利尿薬', name: 'フロセミド', code: 'Furosemide', conc_mg_mL: 10.0, target_dose: 10.0, target_unit: 'mg/kg/day' }
];
...
function initializeApp() {
    ...
    const categories = Array.from(new Set(DRUG_DATA.map(d => d.category.trim())));

    categorySelect.innerHTML = ''; // プルダウンを初期化
...
function populateDrugSelect() {
    ...
    const selectedCategory = categorySelect.value.trim(); // 選択されたカテゴリをtrim()
    drugSelect.innerHTML = ''; // リセット

    const filteredDrugs = DRUG_DATA.filter(d => d.category.trim() === selectedCategory); // データ側のカテゴリもtrim()して比較
...
function updateFixedDoseDisplay() {
    ...
    if (drug) {
        let displayHtml = `目標: 1 mL/hr = **${drug.target_dose} ${drug.target_unit}** (${drug.name})`;
...
function roundToEvenFirstDecimal(value) {
    ...
    if (firstDecimal % 2 !== 0) {
        return (Math.ceil(roundedValue * 10) / 10);
    }
    return roundedValue;
}
// **********************************
...
function calculateDilutionVolume() {
    ...
    // 3. エラーハンドリング
    if (isNaN(weightKg) || weightKg <= 0 || !drug) {
